...
public:
    ...
    void start_client()
    {
        ...
        auto nametype_callback = [&](const flexbuffers::Vector &root)
        {
            ...
            for (int tid = 0; tid < tlen; ++tid)
            {
                ...
                printf(thread_name_c[tid]); printf("\n\t");
        ...
        auto signal_callback = [&](const flexbuffers::Vector &root)
        {
            ...
            for(int t = 1; t < tlen; ++t)
            {
                ...
                for (int i = 0; i < snum; ++i)
                {
                    ...
                    if(search == thread_data[tid].sbuffer.end()) continue;
       ...
       Cli = std::unique_ptr< ClientViewer<>>(new ClientViewer<>(std::string("c0"), std::string("d0"), nametype_callback, signal_callback));
        Cli->init();
    ...
    // {
    //     SignalID sid;
    //     RtDataDynamicRate<10*100*20> sb;
    // };

    struct ThreadBuffer
    {
        // SignalThreadID tid;
        //std::vector<SignalBuffer> tb;
    ...
    // source string exists under the ClientViewer-conn-mt
...
*/
    ...
    auto loop_task = [&](ImGuiIO &io)
    {
        ...
        for (int tid = 0; tid < guib.thread_data.size(); ++tid)
        {
            ...
            {
                if(list[sid] == 1)
                    window((guib.thread_data[tid].thread_name +"/"+ guib.thread_data[tid].signal_name[sid]).c_str(), [&]() { guib.thread_data[tid].splot[sid]->plot(); });
            }
        }
